...
int al_add(ArrayList* pList,void* pElement)
{
    ...
    if(pList!=NULL && pElement != NULL){
        ...
        void** pAux = (pList->pElements) + pList->size;
        *pAux = pElement;
        pList->size++;
...
int al_set(ArrayList* pList, int index,void* pElement)
{
    ...
    if(pList!=NULL && pElement!=NULL && index>=0 && index<=pList->size){
        ...
        if(pList->size==pList->reservedSize && index==pList->size){
            resizeUp(pList);
        }
        if(pList->size==index){
...
int resizeUp(ArrayList* pList)
{
    int returnAux = -1;
    void** aux;
    aux = realloc(pList->pElements,(pList->reservedSize + AL_INCREMENT)*sizeof(void*));
    if(aux==NULL){
